...
rankbwt = function(x, eof = '!') {
  ...
  for (c in bwt_char) {
    # result = tryCatch(
    #                        { is.null(tots[c]) },
    #   warning = function(w){ isInIntots = FALSE },
    #   error   = function(e){ isInIntots = FALSE },
    #   finally =            { isInIntots = is.null(tots[c]) }
    # )
      
      #tryCatch(!is.null(tots[[c]]),FALSE)
    if (!c %in% tots) {
      print(">>> START")
      print(c)
      print(tots)
    ...
    #print(tots)
    #print("---")
    #tots[is.na(tots)] = 0 # replacing NA's with 0's
