...
% -----------------------------------------------------------------------
\section{Part II: Early Computer-Age Methods} % -------------------------
% -----------------------------------------------------------------------
...
The impurity of a node measures the deviation from the predicted behaviour of the node. $\rightarrow$ it's the probability that the prediction is incorrect usually measured by the Gini impurity or the mse (sum of squares error)

\medskip
The Gini impurity is as follows: $$ \sum_{\text{group i}} p(i) \cdot (1 - p(i)) $$ where $p(i)$ is the probability of success of the node. \emph{e.g.} $\sum_{\text{group i}} p(i) \cdot (1 - p(i)) = \frac{5}{95 + 5} \left(\frac{95}{95 + 5}\right) + \frac{10}{150 + 10} \left(\frac{150}{150 + 10}\right)$, where 5 points where wrong about outcome A, 95 right about outcome A, 10 wrong about outcome B and 150 right about outcome B.

% -----------------------------------------------------------------------
\subsection{Life Tables \& Hazard Rates} % ------------------------------

What is the probability of dying/failing at time $i$?

% -----------------------------------------------------------------------
\subsubsection{Discrete time}

$$ S_i = \sum_{j \geq i} f_j = P_r(x \geq i) $$ is the probability of surviving past time $i - 1$, where \\
$S_i$ is the marginal survival rate ($S_{0, i - 1} = S_{i, j}$) \\
$f_j$ probability of dying at time $j$ \\
$x$ length of a lifetime

$$ h_i = \frac{f_i}{S_i} = P_r(x = i | x \geq i) $$ is the probability of dying exactly at time $i$ given survival past time $i - 1$

$$ S_{i, j} = \prod_{k = i}^{j} (1 - h_k) = P(x > j | x \geq i) $$ is the conditional surviving rate past age $j$ given survived time $i - 1$

\medskip
parameter estimates: \\
$\hat{h_i} = \frac{y_i}{n_i}$ death rate at time $i$ \\
$\hat{S_{t_0, j}} = \prod_{k 0 = t_0}^{j} (1 - \hat{h_k})$

\medskip
It can be used to learn the probability of a $t_0$ old to survive past time $t_n$

% -----------------------------------------------------------------------
\subsubsection{Continuous time}

$$ S(t) = \int_t^\infty f(x) dx = P(T \geq t) $$ is the survival function, where $f(x) is a density function (pdf)$. $S(t) = 1 - F(t)$ as it's the reverse cdf (cumulative distribution function)

$$ h(t) = \frac{f(t)}{S(t)} $$ is the hazard rate density function

$$ S(t) = e^{- \int_0^t h(x) dx} $$ is the continuous survival function in terms of the hazard function
...
Used to assign a standard error to a $\hat{\theta}$

\medskip
* given a \emph{iid} sample $\underline{x} = (x_1, \hdots, x_n)$ from an unknown probability distribution F on some sample space $X$

* some algorithm S() can be applied to compute $\hat{\theta} = S(x)$

* let $x_{-i} = (x_1, x_2, \hdots, x_{i - 1}, x_{i + 1}, \hdots, x_n)$ (remove $x_i$)

* so, $\hat{\theta}_{-i} = S(x_{-i})$

$$ \hat{se}_{Jack} = \left[\frac{n - 1}{n} \cdot \sum_{i = 1}^{n} (\hat{\theta}_{-i} - \hat{\theta}(\cdot))^2 \right]^{\frac{1}{2}} $$ where: $\hat{\theta}(\cdot) = \sum_{i = 1}^{n} \frac{\hat{\theta}_{-i}}{n}$

\medskip
i) get each one of the $\hat{\theta}_{-i}$ \\
ii) numerically compute $\hat{\theta}(\cdot)$ \\
iii) do $\hat{\theta}_{-i} - \hat{\theta}(\cdot)$ \\
iv) apply JK equation

\medskip
* works for any distribution
* it depends on local derivatives - non-smooth statistics can result in erradic behaviour

% -----------------------------------------------------------------------
\subsection{Non-parametric Bootstrap} % ---------------------------------

* replaces an unknown distribution $F$ with an estimate $\hat{F} = \underline{x}$

* each bootstrap sample provides a bootstrap replication of $\hat{\theta}^{*,b} = S(\underline{x}^{*,b})$ for $b = 1, 2, \hdots, B$

$$ \hat{se}_{boot} = \left[ \frac{\sum_{b = 1}^B (\hat{\theta}^{*,b} - \hat{\theta}^{*,\cdot})^2}{n - 1} \right] = se(\hat{F}) $$ where $\hat{\theta}^{*,\cdot} = \frac{\sum_{b = 1}^B \hat{\theta}^{*,b}}{B}$
...
$$ \hat{se}_{IJ} = \left(\frac{{D_i}^2}{n^2}\right)^{\frac{1}{2}} $$ computationally, shall be done with a small $\varepsilon$

% -----------------------------------------------------------------------
\subsubsection{Moving Blocks Bootstrap}

Used in time series data, where $x$ occurs in a meaningful order.

\medskip
Let $B_m$ be the set of continuous blocks of length $m$. \emph{e.g.} $B_3 = \{(x_1, x_2, x_3), (x_2, x_3, x_4), \hdots, (x_{n-2}, x_{n-1}, x_n)\}$

\medskip
Choose a m big enough such that the correlation between the $x's$ is negligible

\medskip
Then use regular 1-sample bootstrap to randomly draw $n/m$ block numbers

$$ \hat{se}_{bootB} = \left[\sum_{b = 1}^B \frac{\hat{\theta}^{*,b} - \hat{\theta}^{*,\cdot}}{B - 1} \right]^{\frac{1}{2}} $$ where $\hat{\theta}^{*,\cdot} = \sum_{b = 1}^B \frac{\hat{\theta}^{*,b}}{B}$

% -----------------------------------------------------------------------
\subsubsection{Multisample Bootstrap}

Used when subjects in data are classified in 2 or more groups; and want to compute an estimator (e.g. median) for each group and then compare between them.

\medskip
Do not sample from the $n_total$ observations, instead re-sample with replacement $n_1$ samples of group 1 and $n_2$ samples of group 2 separately.

\medskip
Then perform t-student for the means of the estimator. (as each bootstap sample provides a bootstrap replication of S())

% -----------------------------------------------------------------------
\subsubsection{Parametric Bootstrap}

In regular bootstrap sample $\underline{x}$ is used as $\hat{F}$; where every outcome/category has the same probability ($1/n$), by keeping the y-axis at a fixed width histogram-wise.

\medskip
Parametric Bootstrap is about keeping categories in the x-axis at a fixed width histogram-wise.

* it uses $f_{\hat{\theta}} (x)$ as $\hat{F}$, therefore the vector of weights $P$ is given by $f_{\hat{\theta}} (x)$
