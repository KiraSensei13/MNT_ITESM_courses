...
We begin by defining a function that will contain the instructions for the operations to be performed in order to obtain the approximation:
```{r}
library(pracma)
...
Defining the approximations to be plotted:
```{r}  
...
We define the input function to be approximated:
```{r}
f0 <- function(x) {
  res = sin(x)
...
Calling for the plotting function:
```{r}
